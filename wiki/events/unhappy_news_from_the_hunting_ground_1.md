This page has the same name as others. For full listing see bottom of [the base page](unhappy_news_from_the_hunting_ground.md).

#Information
 - Title: Unhappy news from the hunting ground
 - ID: 8001
#Description
Unhappy news from the hunting ground
#Mean Time to Happen:
Base time = 5000 months
 - Multiplied by 0.95 if has owner has innovativeness ideas is 4
 - Multiplied by 0.95 if has owner has innovativeness ideas is 5
 - Multiplied by 0.95 if has owner has innovativeness ideas is 6
 - Multiplied by 0.95 if has owner has innovativeness ideas is 7
 - Multiplied by 2.0 if has owner has government is theocracy
 - Multiplied by 0.95 if has any neighbor province has religion is totemism

#Options

___
##Still at least we are tolerant.

###Efects:<ul><li>change religion = totemism</li></ul>
